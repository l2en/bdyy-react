...
var mapControl = {
    ...
    */
    ...
    addTrackPointOverlay(point, type) {
        // var myCompOverlay = new this.trackPointOverlay(point, type);
        let tmpE = new this.initTrackPointOverlay()
        var myCompOverlay = tmpE.trackPointOverlay(point, type);
        map.addOverlay(myCompOverlay);
    },
    ...
    */
    removeTrackPointOverlay(type) {
        var overlays = map.getOverlays();
        var length = overlays.length;
        var trackPointOverlays = [];
        for (var i = 0; i < length; i++) {
            if (overlays[i].type === type) {
                trackPointOverlays.push(overlays[i]);
            }
        }
        for (var j = 0; j < trackPointOverlays.length; j++) {
    ...
    setMonitorInfoBox(data) {
        ...
        this.monitorInfoBox = new BMapLib.InfoBox(
            ...
            infoContentFrontArr.concat(infoContentNextArr).join(''),
            {
                ...
                alignBottom: false,
                closeIconUrl: '../../static/images/closeinfowindow.png'
            }
        );
    ...
    setEntityMarker(data, service_type) {
        ...
        if (service_type === 1) {
            ...
            switch (status.substring(0, 2)) {
                case '离线':
                    iconUrl = '../../static/images/caroffnorth.png';
                    break;
                case '静止':
                    iconUrl = '../../static/images/carstaticnorth.png';
                    break;
                default:
                    iconUrl = '../../static/images/carrunnorth.png';
                    break;
            }
        } else {
            ...
            switch (status.substring(0, 2)) {
                case '离线':
                    iconUrl = '../../static/images/othertypeoffline.png';
                    break;
                case '静止':
                    iconUrl = '../../static/images/othertypestatic.png';
                    break;
                default:
                    iconUrl = '../../static/images/othertype.png';
                    break;
            }
        }
     ...
     */
    removeTrackInfoBox() {
        console.log('mapControl的removeTrackInfoBox')
