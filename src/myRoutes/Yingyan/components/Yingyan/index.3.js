...
class Yingyan extends Component {
  ...
  componentDidMount() {
    setTimeout(() => {
      this.initMap()
      this.onSelecttrack('测试点5')
      $timelineProgress = $('#timelineProgress');
      $runPart = $('#runPart');
      console.log('====0000', $timelineProgress.offset());
   ...
   */
  onSelecttrack = (name) => {
    if (!name || CONSTANT.track.trackSearching === 1) return;
    CONSTANT.track.trackSearching = 1;
    var tempTimeArr = [];
    var partTime = Math.floor((CONSTANT.track.end_time - CONSTANT.track.start_time) / 6);

    CONSTANT.track.trackRoutePointData = [];
    CONSTANT.track.trackRouteSortData = [];
    CONSTANT.track.trackRouteNoZero = [];
    CONSTANT.track.selectTrack = name;

    for (var i = 0; i < 6; i++) {
      tempTimeArr[i] = {
        start_time: CONSTANT.track.start_time + i * partTime,
        end_time: CONSTANT.track.start_time + (i + 1) * partTime - 1,
        index: i
      }
    }

    tempTimeArr[5].end_time = CONSTANT.track.end_time;
    var params = {
      'entity_name': CONSTANT.track.selectTrack,
      'service_id': service_id,
      'ak': ak,
      'simple_return': 0,
      'page_size': 5000,
      'is_processed': CONSTANT.track.trackProcess.is_processed,
      'process_option': 'need_denoise=' + CONSTANT.track.trackProcess.need_denoise + ',' +
        'need_vacuate=' + CONSTANT.track.trackProcess.need_vacuate + ',' +
        'need_mapmatch=' + CONSTANT.track.trackProcess.need_mapmatch + ',' +
        'transport_mode=' + CONSTANT.track.transport_mode[CONSTANT.track.trackProcess.transport_mode - 1]
    };
...
*/
  reTrackRoute = (paramsr, page_index, count) => {
    ...
    paramsr.entity_name = '测试点5'
    paramsr.start_time = '1539129600'
    paramsr.end_time = '1539169200'
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        CONSTANT.track.trackRouteDataCount = CONSTANT.track.trackRouteDataCount + 1;
        if (data.status === 0) {
          CONSTANT.track.trackRouteSortData.push({ index: count, track: data });
          if (CONSTANT.track.trackRouteDataCount === 12) {
            CONSTANT.track.trackRouteDataCount = 0;
            CONSTANT.track.trackRouteSortData.sort(function (a, b) { return a.index - b.index });

            for (var i = 0; i < 12; i++) {
              CONSTANT.track.trackRoutePointData = CONSTANT.track.trackRoutePointData.concat(CONSTANT.track.trackRouteSortData[i].track.points);
            }
            CONSTANT.track.trackRoutePointData.map(function (item) {
              if (item.longitude > 1 && item.latitude > 1) {
                CONSTANT.track.trackRouteNoZero.push(item);
              }
            });
            draw.first = true;
            CONSTANT.track.trackSearching = 0;
            console.log('==========即将显示的点', CONSTANT.track.trackRouteNoZero)
            _this.drawTrack(CONSTANT.track.trackRouteNoZero);
            _this.listenTrackRoute(CONSTANT.track.trackRouteNoZero);
          }
        }
        if (CONSTANT.track.trackRouteDataCount === 12) {
          CONSTANT.track.trackRouteDataCount = 0;
...
*/
  drawTrack = (data, starttime, endtime) => {
    if (!data) {
      data = CONSTANT.track.trackRouteNoZero;
    }
    var totalPoints = [];
    var viewportPoints = [];
    ...
    for (var i = 0; i < data.length; i++) {
      if (data[i].loc_time >= starttime && data[i].loc_time <= endtime) {
        ...
        tempPoint.print = CONSTANT.track.selectTrack;
    ...
    console.log('整理后即将显示的点信息', totalPoints)
    ...
    window.pointCollection = new BMap.PointCollection(totalPoints, options);  // 初始化PointCollection   这里是渲染重点！！！！！
 ...
 */
  ...
  onBehavioranalysis = () => {
    if (CONSTANT.track.selectTrack === '' || CONSTANT.track.analysisbehaviorSearching === 1) return;
    CONSTANT.track.analysisbehaviorSearching = 1;
    var entity_name = CONSTANT.track.selectTrack;
    var tempTimeArr = [];
    CONSTANT.track.trackBehaviorSortData = [];
    CONSTANT.track.trackBehaviorPointData = {
      harsh_acceleration: [],
      harsh_breaking: [],
      harsh_steering: [],
      speeding: []
    ...
    var reBehavior = function (paramsr) {
      ...
      var search = function (paramsearch, counta) {
        $.ajax({
          ...
          success: function (data) {
            if (data.status === 0) {
              CONSTANT.track.trackBehaviorSortData.push({ index: counta, data: data });
              if (++CONSTANT.track.trackBehaviorDataCount === 6) {
                CONSTANT.track.trackBehaviorDataCount = 0;
                CONSTANT.track.trackBehaviorSortData.sort(function (a, b) { return a.index - b.index });

                for (var i = 0; i < 6; i++) {
                  CONSTANT.track.trackBehaviorPointData.harsh_acceleration = CONSTANT.track.trackBehaviorPointData.harsh_acceleration.concat(
                    CONSTANT.track.trackBehaviorSortData[i].data.harsh_acceleration
                  );
                  CONSTANT.track.trackBehaviorPointData.harsh_breaking = CONSTANT.track.trackBehaviorPointData.harsh_breaking.concat(
                    CONSTANT.track.trackBehaviorSortData[i].data.harsh_breaking
                  );
                  CONSTANT.track.trackBehaviorPointData.harsh_steering = CONSTANT.track.trackBehaviorPointData.harsh_steering.concat(
                    CONSTANT.track.trackBehaviorSortData[i].data.harsh_steering
                  );
                  CONSTANT.track.trackBehaviorPointData.speeding = CONSTANT.track.trackBehaviorPointData.speeding.concat(
                    CONSTANT.track.trackBehaviorSortData[i].data.speeding
                  );
                }
                this.drawAnalysisBehavior(CONSTANT.track.trackBehaviorPointData);
                CONSTANT.track.analysisbehaviorSearching = 0;
              }
            }
          },
          error: function () {
  ...
  onGetstaypoint = () => {
    const _this = this;
    if (CONSTANT.track.selectTrack === '' || CONSTANT.track.staypointSearching === 1) return;
    CONSTANT.track.staypointSearching = 1;
    var entity_name = CONSTANT.track.selectTrack;
    var tempTimeArr = [];
    CONSTANT.track.trackStayRouteSortData = [];
    CONSTANT.track.trackStayRoutePointData = [];
    var partTime = Math.floor((CONSTANT.track.end_time - CONSTANT.track.start_time) / 6);
    for (var i = 0; i < 6; i++) {
      tempTimeArr[i] = {
        start_time: CONSTANT.track.start_time + i * partTime,
    ...
    var reTrackRoute = function (paramsr) {
      ...
      var search = function (paramsearch, counta) {
        $.ajax({
          ...
          success: function (data) {
            if (data.status === 0) {
              CONSTANT.track.trackStayRouteSortData.push({ index: counta, data: data });
              if (++CONSTANT.track.trackStayRouteDataCount === 6) {
                CONSTANT.track.trackStayRouteDataCount = 0;
                CONSTANT.track.trackStayRouteSortData.sort(function (a, b) { return a.index - b.index });

                for (var i = 0; i < 6; i++) {
                  if (CONSTANT.track.trackStayRouteSortData[i].data.stay_points !== undefined) {
                    CONSTANT.track.trackStayRoutePointData = CONSTANT.track.trackStayRoutePointData.concat(CONSTANT.track.trackStayRouteSortData[i].data.stay_points);
                  }
                }
                this.drawStaypoint(CONSTANT.track.trackStayRoutePointData);
                CONSTANT.track.staypointSearching = 0;
              }
            }
          },
          error: function () {
  ...
  */
  drawStaypoint = (data) => {
    if (!data) data = CONSTANT.track.trackStayRoutePointData;
    var starttime = draw.starttime;
    var endtime = draw.endtime;
    ...
    updateAnalysisBehavior(CONSTANT.track.behaviorCheck);
  }
...
*/
  updateAnalysisBehavior = (data) => {
    data = data || CONSTANT.track.behaviorCheck;
  ...
  */
  onGetaddress = (point) => {
    ...
    $.ajax({
      ...
      success: function (data) {
        var infoBoxObject = this.getTrackPointInfo(data, point);
...
*/
  ...
  */
  drawAnalysisBehavior = (data) => {
    if (!data) data = CONSTANT.track.trackBehaviorPointData;
    var starttime = draw.starttime;
    var endtime = draw.endtime;

    CONSTANT.track.behavior.behaviorSpeeding = data.speeding.length;
    CONSTANT.track.behavior.behaviorAcceleration = data.harsh_acceleration.length + data.harsh_breaking.length;
    CONSTANT.track.behavior.behaviorSteering = data.harsh_steering.length;
    $controlItemNum.eq(0).text(CONSTANT.track.behavior.behaviorSpeeding);
    $controlItemNum.eq(1).text(CONSTANT.track.behavior.behaviorAcceleration);
    $controlItemNum.eq(2).text(CONSTANT.track.behavior.behaviorSteering);
