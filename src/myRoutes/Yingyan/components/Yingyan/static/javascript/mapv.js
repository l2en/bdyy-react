...
DataSet.prototype.get = function (args) {
    ...
    var start = new Date();
    // TODO: 涓嶄慨鏀瑰師濮嬫暟鎹紝鍦ㄦ暟鎹笂鎸傝浇鏂扮殑鍚嶇О锛屾瘡娆′慨鏀规暟鎹洿鎺ヤ慨鏀规柊鍚嶇О涓嬬殑鏁版嵁锛屽彲浠ョ渷鍘籨eepCopy
...
function getCurveByTwoPoints(obj1, obj2, count) {
  ...
  var count = count || 40; // 鏇茬嚎鏄敱涓€浜涘皬鐨勭嚎娈电粍鎴愮殑锛岃繖涓〃绀鸿繖涓洸绾挎墍鏈夊埌鐨勬姌绾跨殑涓暟
  var isFuture = false;
...
var MapHelper = function () {
    function MapHelper(id, type, opt) {
        ...
        if (!id || !type) {
            console.warn('id 鍜� type 涓哄繀濉」');
            return false;
        }

        if (type == 'baidu') {
            if (!BMap) {
                console.warn('璇峰厛寮曞叆鐧惧害鍦板浘JS API');
                return false;
            }
        } else {
            console.warn('鏆備笉鏀寔浣犵殑鍦板浘绫诲瀷');
        }
        this.type = type;
...
/**
 * 涓€鐩磋鐩栧湪褰撳墠鍦板浘瑙嗛噹鐨凜anvas瀵硅薄
 *
 * @author nikai (@鑳栧槦鍢熺殑楠ㄥご, nikai@baidu.com)
 *
 * @param 
 * {
 *     map 鍦板浘瀹炰緥瀵硅薄
...
/**
 * 鏍规嵁2鐐硅幏鍙栬搴�
 * @param Array [123, 23] 鐐�1
 * @param Array [123, 23] 鐐�2
 * @return angle 瑙掑害,涓嶆槸寮у害
 */
...
var drawText = {
    draw: function draw(context, dataSet, options) {
        ...
        if (options.avoid) {
            ...
            for (var i = 0, len = data.length; i < len; i++) {
                ...
                // 鏍规嵁鏂囨湰瀹藉害鍜岄珮搴﹁皟鏁磝锛寉浣嶇疆锛屼娇寰楃粯鍒舵枃鏈椂鍊欏潗鏍囩偣鍦ㄦ枃鏈腑蹇冪偣锛岃繖涓绠楀嚭鐨勬槸宸︿笂瑙掑潗鏍�
    ...
    /*
     *  褰撳墠鏂囧瓧鍖哄煙鍜屽凡鏈夌殑鏂囧瓧鍖哄煙鏄惁鏈夐噸鍙犻儴鍒�
     */
...
function isRectOverlay(rect1, rect2) {
    //minx銆乵iny 2涓煩褰㈠彸涓嬭鏈€灏忕殑x鍜寉
    //maxx銆乵axy 2涓煩褰㈠乏涓婅鏈€澶х殑x鍜寉
...
var AnimationLayer = function (_BaseLayer) {
    ...
    createClass(AnimationLayer, [{
        ...
        value: function _canvasUpdate() {
            ...
            var nwMc = new BMap.Pixel(mcCenter.x - map.getSize().width / 2 * zoomUnit, mcCenter.y + map.getSize().height / 2 * zoomUnit); //宸︿笂瑙掑ⅷ鍗℃墭鍧愭爣
